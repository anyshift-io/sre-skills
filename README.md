...
[`sqs-queue-auditor`](./skills/sqs-queue-auditor/), [`iam-deceptive-escalation-auditor`](./skills/iam-deceptive-escalation-auditor/), and [`sg-deceptive-reachability-auditor`](./skills/sg-deceptive-reachability-auditor/) are built out, each with fixture-based replay tests and a committed control-vs-treatment lift eval; the rest are *planned*. [`kubectl-investigator`](./skills/kubectl-investigator/) stays as the methodology-shaped reference template: it shows the directory shape, the worked-example format, and the fixture-based replay tests every skill above follows.
