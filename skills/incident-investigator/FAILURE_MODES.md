# Failure modes: `incident-investigator`
...
Step 1 picks T0 as the first user-visible symptom. When T0 is set 20 minutes late, the change-surface bisection in step 2 misses the actual triggering change, and the agent confidently exonerates a deploy that did in fact cause the incident.
...
Temporal coincidence with a deploy is the easiest path for the agent to find and the easiest to over-trust. The deploy is often correlated but not causal (e.g. the deploy and the failure both follow from a third event like a capacity event or a config change applied minutes earlier).

**Mitigation in the methodology**: step 4 requires three independent signals. For the deploy-correlator path specifically, two of those signals must be drawn from the deploy diff itself (e.g. the diff touches the failing code path, the canary version shows asymmetry) rather than just from timing.

## Operational failure modes

### O1. Stale or missing change-event sources

Step 2 assumes the agent can query deploy events, IAM audit logs, and config-change history. When one of these sources is missing or stale by more than the window, the bisection silently omits a category of changes.
