...
- **Service**: `payments-api` calls `api.stripe.com` for charge processing.
...
|---|---|---|---|
| CI/CD | (none) | | No code deploys in window. Most recent `payments-api` deploy was 3 days ago. |
| Terraform | (none) | | |
| IAM | (none) | | |
| Feature flags | (none) | | |
| Cron / batch | (none) | | |
...
- **OOM**: `payments-api` RSS p95 flat at ~190 MB against a 512 MB limit. No `OOMKilled` events. No match.
...
4. **Scale**: not applicable (the limit is external, not internal capacity).
