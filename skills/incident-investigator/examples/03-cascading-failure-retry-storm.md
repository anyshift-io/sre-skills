...
|---|---|---|---|
| CI/CD | (none) | | No code deploys in window. Most recent `payments-api` deploy was 9 days ago. |
| Terraform | (none) | | |
| IAM | (none) | | |
| Feature flags | (none) | | |
| Cron / batch | (none) | | |
...
5. **Manual intervention** (restart `payments-api` pods to drain the saturated queue): only if circuit-breaker and traffic-shift are blocked. Does not address cause; the pool will saturate again within minutes if `ledger-svc` is still slow.
