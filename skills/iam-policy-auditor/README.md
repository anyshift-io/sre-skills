...
A reference run with Claude Sonnet 4.6 (N=3 trials per cell, all 11 fixtures, LLM-as-judge against the 7-item rubric) measured **+1.18 / 7 (+21%) lift** of an agent loaded with this `SKILL.md` over an agent given the same policy JSON with no methodology (control mean 5.58 → treatment 6.76). Treatment beats control on 8 of 11 fixtures, ties on 2, and loses on 1 (a noise loss on the NotAction fixture both conditions handle well).

The lift concentrates where the skill earns its keep: teaching the agent **when not to fire**. Unskilled Sonnet passes the "no false positives" rubric item only **18%** of the time — it manufactures a finding on a clean policy and calls a correctly-narrowed wildcard "public". The skill raises that to 85%, and on the clean control (fixture 11) it swings the score from 3.00 to a clean 7.00 (+4.00). The honest surprise: **cross-statement reasoning showed no lift** — Sonnet 4.6 already chains `PassRole`+`RunInstances` across statements unaided — so on this model the value is severity calibration (scoped vs unscoped PassRole) and the boundary section, not the combo detection itself. See [`tests/eval/README.md`](./tests/eval/README.md) for the per-fixture table, the per-rubric-item breakdown, and the caveats. Reproduce with `python tests/eval/run_eval.py --trials 5` (needs `ANTHROPIC_API_KEY` and `pip install anthropic`).
