...
set -u
cd "$(dirname "$0")/../.."   # -> skills/iam-policy-auditor
TRIALS="${1:-1}"
MAX_ATTEMPTS="${2:-12}"
...
FIXTURES=(
  01-admin-star
  02-passrole-runinstances
  03-create-policy-version
  04-update-function-code
  05-not-action-allow
  06-attach-policy-self
  07-update-assume-role
  08-service-wildcard-exfil
  09-public-trust-policy
  10-scoped-passrole-boundary
  11-clean-least-privilege
)
...
try:
    ...
    sys.exit(1)
c = sum(1 for r in d if r["fixture"] == fx and r["condition"] == "control")
t = sum(1 for r in d if r["fixture"] == fx and r["condition"] == "treatment")
